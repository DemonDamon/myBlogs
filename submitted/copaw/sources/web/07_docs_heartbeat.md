...
如果你还没看过 [项目介绍](/docs/intro)，建议先看一眼那里对「心跳」和「频道」的说明。
...
> 需要「每天 9 点发早安」「每 2 小时问待办并发到钉钉」这类多条任务？用 [CLI](/docs/cli) 的 `copaw cron create` 做定时任务，不用心跳。
...
* [频道配置](/docs/channels) — 先接好频道，target=last 才有「上次频道」可发
* [CLI](/docs/cli) — init 时配置心跳、cron 定时任务
