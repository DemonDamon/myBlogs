...
* **定时执行** — 按你的配置自动运行任务。
* **能力由 Skills 决定，有无限可能** — 内置定时任务、PDF 与表单、Word/Excel/PPT 文档处理、新闻摘要、文件阅读等，还可在 [Skills](/docs/skills) 中自定义扩展。
* **数据全在本地** — 不依赖第三方托管。
...
1. **[快速开始](/docs/quickstart)** — 用三条命令把服务跑起来。
2. **[控制台](/docs/console)** — 服务启动后，**在配置频道之前**，可以先在这里（浏览器打开服务根地址）与 CoPAW 对话，也可以在这里配置 Agent；先看控制台有助于理解 CoPAW 怎么用。
3. **按需配置与使用**：
   * [频道配置](/docs/channels) — 接入钉钉 / 飞书 / QQ / Discord / iMessage，在对应 app 里与 CoPaw 对话；
   * [心跳](/docs/heartbeat) — 配置定时自检或摘要（可选）；
   * [CLI](/docs/cli) — 初始化、定时任务、清空工作目录等命令；
   * [Skills](/docs/skills) — 了解与扩展 CoPaw 能力；
   * [配置与工作目录](/docs/config) — 工作目录与配置文件说明。