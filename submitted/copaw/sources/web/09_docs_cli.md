...
> 还不清楚「频道」「心跳」「定时任务」是什么？先看 [项目介绍](/docs/intro)。
...
一个用于对话、频道、定时任务、技能、模型等的 Web 管理界面。详见 [控制台](/docs/console)。
...
> 详见 [配置 — 模型提供商](/docs/config#%E6%A8%A1%E5%9E%8B%E6%8F%90%E4%BE%9B%E5%95%86)。
...
> 详见 [配置 — 环境变量](/docs/config#%E7%8E%AF%E5%A2%83%E5%8F%98%E9%87%8F)。
...
> 各平台凭据的获取步骤，请看 [频道配置](/docs/channels)。
...
> 内置技能说明和自定义技能编写方法，请看 [技能](/docs/skills)。
...
| `copaw init` | — | 否 |
| `copaw app` | — | —（启动服务本身） |
| `copaw models` | `list` · `config` · `config-key` · `set-llm` | 否 |
...
* [项目介绍](/docs/intro) —— CoPaw 可以做什么
* [控制台](/docs/console) —— Web 管理界面
* [频道配置](/docs/channels) —— 钉钉、飞书、iMessage、Discord、QQ 详细步骤
* [心跳](/docs/heartbeat) —— 定时自检/摘要
* [技能](/docs/skills) —— 内置技能与自定义技能
* [配置与工作目录](/docs/config) —— 工作目录与 config.json